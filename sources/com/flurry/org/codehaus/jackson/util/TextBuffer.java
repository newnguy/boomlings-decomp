...
public final class TextBuffer {
    ...
    private void expand(int i) {
        ...
        char[] _charArray = _charArray(Math.min((int) MAX_SEGMENT_LEN, length + i));
        this._currentSize = 0;
        this._currentSegment = _charArray;
    }

    private final char[] findBuffer(int i) {
        return this._allocator != null ? this._allocator.allocCharBuffer(BufferRecycler.CharBufferType.TEXT_BUFFER, i) : new char[Math.max(i, (int) MIN_SEGMENT_LEN)];
    }
    ...
    public char[] expandCurrentSegment() {
        char[] cArr = this._currentSegment;
        int length = cArr.length;
        this._currentSegment = _charArray(length == MAX_SEGMENT_LEN ? 262145 : Math.min((int) MAX_SEGMENT_LEN, (length >> 1) + length));
    ...
    public char[] finishCurrentSegment() {
        ...
        char[] _charArray = _charArray(Math.min(length + (length >> 1), (int) MAX_SEGMENT_LEN));
        this._currentSize = 0;
        this._currentSegment = _charArray;
        return _charArray;
    }

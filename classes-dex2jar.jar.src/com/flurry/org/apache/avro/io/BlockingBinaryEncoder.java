    ...
    while (true) {
      ...
      if (blockingBinaryEncoder$BlockedValue.items > 0) {
        ...
        if (blockingBinaryEncoder$BlockedValue.start == 0 && (this.blockStack[this.stackTop - 1]).state != BlockingBinaryEncoder$BlockedValue$State.REGULAR) {
          ...
          k += BinaryData.encodeInt(j, this.headerBuffer, k);
          if (this.buf.length >= this.pos + k) {
            this.pos += k;
            int m = blockingBinaryEncoder$BlockedValue.start;
            System.arraycopy(this.buf, m, this.buf, m + k, j);
            System.arraycopy(this.headerBuffer, 0, this.buf, m, k);
          } else {
            compact();
            continue;
          } 
...
/* Location:              C:\Users\walle\Downloads\boomlings-1-20 (2)\classes-dex2jar.jar!\com\flurry\org\apache\avro\io\BlockingBinaryEncoder.class

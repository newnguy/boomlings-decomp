...
public class BinaryDecoder extends Decoder {
  ...
  public double readDouble() {
    ensureBounds(8);
    byte b6 = this.buf[this.pos];
    byte b8 = this.buf[1 + this.pos];
    byte b2 = this.buf[this.pos + 2];
    byte b7 = this.buf[this.pos + 3];
    byte b4 = this.buf[this.pos + 4];
    byte b3 = this.buf[this.pos + 5];
    byte b1 = this.buf[this.pos + 6];
    byte b5 = this.buf[this.pos + 7];
    if (this.pos + 8 > this.limit)
      throw new EOFException(); 
    this.pos += 8;
    long l = ((b8 & 0xFF) << 8 | b6 & 0xFF | (b2 & 0xFF) << 16 | (b7 & 0xFF) << 24);
    return Double.longBitsToDouble((b4 & 0xFF | (b3 & 0xFF) << 8 | (b1 & 0xFF) << 16 | (b5 & 0xFF) << 24) << 32L | l & 0xFFFFFFFFL);
  }
  ...
  public float readFloat() {
    ensureBounds(4);
    byte b1 = this.buf[this.pos];
    byte b4 = this.buf[1 + this.pos];
    byte b2 = this.buf[this.pos + 2];
    byte b3 = this.buf[this.pos + 3];
    if (this.pos + 4 > this.limit)
      throw new EOFException(); 
    this.pos += 4;
    return Float.intBitsToFloat((b4 & 0xFF) << 8 | b1 & 0xFF | (b2 & 0xFF) << 16 | (b3 & 0xFF) << 24);
  }
...
/* Location:              C:\Users\walle\Downloads\boomlings-1-20 (2)\classes-dex2jar.jar!\com\flurry\org\apache\avro\io\BinaryDecoder.class

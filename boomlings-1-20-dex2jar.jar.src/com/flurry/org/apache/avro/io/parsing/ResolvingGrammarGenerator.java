...
public class ResolvingGrammarGenerator extends ValidatingGrammarGenerator {
  ...
  public static void encode(Encoder paramEncoder, Schema paramSchema, JsonNode paramJsonNode) {
    ...
    switch (paramSchema.getType()) {
      ...
      case null:
        ...
        while (true) {
          if (iterator2.hasNext()) {
            ...
            JsonNode jsonNode1 = paramJsonNode.get(str1);
            jsonNode = jsonNode1;
            if (jsonNode1 == null)
              jsonNode = field.defaultValue(); 
            if (jsonNode == null)
              throw new AvroTypeException("No default value for: " + str1); 
      ...
      case null:
        ...
        iterator = paramJsonNode.iterator();
        while (true) {
          if (!iterator.hasNext())
            paramEncoder.writeArrayEnd(); 
          JsonNode jsonNode1 = iterator.next();
          paramEncoder.startItem();
          encode(paramEncoder, schema1, jsonNode1);
        } 
      case null:
        paramEncoder.writeMapStart();
        paramEncoder.setItemCount(iterator.size());
        schema2 = schema1.getValueType();
        iterator1 = iterator.getFieldNames();
        while (true) {
          ...
          encode(paramEncoder, schema2, iterator.get(str));
        } 
      case null:
        paramEncoder.writeIndex(0);
        encode(paramEncoder, str.getTypes().get(0), (JsonNode)iterator);
      case null:
        if (!iterator.isTextual())
          throw new AvroTypeException("Non-string default value for fixed: " + iterator); 
        arrayOfByte2 = iterator.getTextValue().getBytes("ISO-8859-1");
        if (arrayOfByte2.length != str.getFixedSize()) {
          int i;
          byte[] arrayOfByte = new byte[str.getFixedSize()];
          if (str.getFixedSize() > arrayOfByte2.length) {
  ...
  private Symbol resolveRecords(Schema paramSchema1, Schema paramSchema2, Map<ResolvingGrammarGenerator$LitS2, Symbol> paramMap) {
    ...
    if (symbol2 == null) {
      ...
      int j = list2.size();
      Iterator<Schema.Field> iterator = list2.iterator();
      int i = 0;
      while (true) {
        Iterator<Schema.Field> iterator1;
        if (!iterator.hasNext()) {
            ...
            if (!iterator.hasNext()) {
              ...
              while (true) {
                if (!iterator.hasNext()) {
                  ...
                  while (true) {
                    if (iterator1.hasNext()) {
                      Schema.Field field4 = iterator1.next();
                      if (paramSchema1.getField(field4.name()) == null) {
                        byte[] arrayOfByte = getBinary(field4.schema(), field4.defaultValue());
                        arrayOfSymbol[--i] = new Symbol$DefaultStartAction(arrayOfByte);
                        arrayOfSymbol[--i] = generate(field4.schema(), field4.schema(), paramMap);
                ...
                Schema.Field field3 = iterator1.getField(field1.name());
                if (field3 == null) {
                  arrayOfSymbol[--i] = new Symbol$SkipAction(generate(field1.schema(), field1.schema(), paramMap));
                  continue;
                } 
                arrayOfSymbol[--i] = generate(field1.schema(), field3.schema(), paramMap);
              } 
              break;
            } 
            Schema.Field field2 = iterator.next();
            if (paramSchema1.getField(field2.name()) == null) {
              if (field2.defaultValue() == null) {
                symbol1 = Symbol.error("Found " + paramSchema1.toString(true) + ", expecting " + iterator1.toString(true));
                paramMap.put(field1, symbol1);
                continue;
              } 
              symbol1[i] = (Symbol)field2;
              j += 3;
              i++;
            } 
          } 
        } 
        Schema.Field field = iterator1.getField(((Schema.Field)iterator.next()).name());
        if (field != null) {
...
/* Location:              C:\Users\walle\Downloads\boomlings-1-20 (2)\boomlings-1-20-dex2jar.jar!\com\flurry\org\apache\avro\io\parsing\ResolvingGrammarGenerator.class

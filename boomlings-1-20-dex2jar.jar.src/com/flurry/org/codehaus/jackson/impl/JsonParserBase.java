...
public abstract class JsonParserBase extends JsonParserMinimalBase {
  ...
  private final void _parseSlowFloatValue(int paramInt) {
    if (paramInt == 16) {
      try {
        ...
        _wrapError("Malformed numeric value '" + this._textBuffer.contentsAsString() + "'", numberFormatException);
      } 
      return;
    } 
  ...
  private final void _parseSlowIntValue(int paramInt1, char[] paramArrayOfchar, int paramInt2, int paramInt3) {
    ...
    try {
      ...
      _wrapError("Malformed numeric value '" + str + "'", numberFormatException);
    } 
  }
  ...
  protected final int _decodeBase64Escape(Base64Variant paramBase64Variant, int paramInt1, int paramInt2) {
    ...
    if (c <= ' ' && paramInt2 == 0)
      return -1; 
    int i = paramBase64Variant.decodeBase64Char(c);
    paramInt1 = i;
    if (i < 0)
      throw reportInvalidBase64Char(paramBase64Variant, c, paramInt2); 
    return paramInt1;
  }
  ...
  public JsonLocation getCurrentLocation() {
    ...
    return new JsonLocation(this._ioContext.getSourceReference(), this._currInputProcessed + this._inputPtr - 1L, this._currInputRow, j - i + 1);
  }
  ...
  public JsonParser.NumberType getNumberType() {
    ...
    return (this._currToken == JsonToken.VALUE_NUMBER_INT) ? (((this._numTypesValid & 0x1) != 0) ? JsonParser.NumberType.INT : (((this._numTypesValid & 0x2) != 0) ? JsonParser.NumberType.LONG : JsonParser.NumberType.BIG_INTEGER)) : (((this._numTypesValid & 0x10) != 0) ? JsonParser.NumberType.BIG_DECIMAL : JsonParser.NumberType.DOUBLE);
  }
  ...
  public Number getNumberValue() {
    if (this._numTypesValid == 0)
      _parseNumericValue(0); 
    if (this._currToken == JsonToken.VALUE_NUMBER_INT)
      return (Number)(((this._numTypesValid & 0x1) != 0) ? Integer.valueOf(this._numberInt) : (((this._numTypesValid & 0x2) != 0) ? Long.valueOf(this._numberLong) : (((this._numTypesValid & 0x4) != 0) ? this._numberBigInt : this._numberBigDecimal))); 
    if ((this._numTypesValid & 0x10) != 0)
      return this._numberBigDecimal; 
    if ((this._numTypesValid & 0x8) == 0)
      _throwInternal(); 
  ...
  protected IllegalArgumentException reportInvalidBase64Char(Base64Variant paramBase64Variant, int paramInt1, int paramInt2) {
    return reportInvalidBase64Char(paramBase64Variant, paramInt1, paramInt2, (String)null);
  }
  ...
  protected IllegalArgumentException reportInvalidBase64Char(Base64Variant paramBase64Variant, int paramInt1, int paramInt2, String paramString) {
    String str1;
    if (paramInt1 <= 32) {
      str1 = "Illegal white space character (code 0x" + Integer.toHexString(paramInt1) + ") as character #" + (paramInt2 + 1) + " of 4-char base64 unit: can only used between units";
    } else if (str1.usesPaddingChar(paramInt1)) {
      str1 = "Unexpected padding character ('" + str1.getPaddingChar() + "') as character #" + (paramInt2 + 1) + " of 4-char base64 unit: padding only legal as 3rd or 4th character";
    } else if (!Character.isDefined(paramInt1) || Character.isISOControl(paramInt1)) {
      str1 = "Illegal character (code 0x" + Integer.toHexString(paramInt1) + ") in base64 content";
    } else {
      str1 = "Illegal character '" + (char)paramInt1 + "' (code 0x" + Integer.toHexString(paramInt1) + ") in base64 content";
    } 
    String str2 = str1;
    if (paramString != null)
  ...
  protected final JsonToken reset(boolean paramBoolean, int paramInt1, int paramInt2, int paramInt3) {
    return (paramInt2 < 1 && paramInt3 < 1) ? resetInt(paramBoolean, paramInt1) : resetFloat(paramBoolean, paramInt1, paramInt2, paramInt3);
  }
...
/* Location:              C:\Users\walle\Downloads\boomlings-1-20 (2)\boomlings-1-20-dex2jar.jar!\com\flurry\org\codehaus\jackson\impl\JsonParserBase.class
